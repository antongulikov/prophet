...
int main(int argc, char* argv[]) {
    ...
    // building basic BenchProgram class and assing it to var P
    ...
    // test verifying with perfect logging
    if (!SkipVerify) {
    ...
    // get localizer, usually "profile"
    std::string localizer = config->getStr("localizer");
    // get bugged files set
   ...
   if (InitOnly) {
    ...
    // read feature para vector obtained from learning stage
    ...
    if (Timeout.getValue() != 0)
        E.setTimeoutLimit((unsigned long long) std::round(Timeout.getValue() * 60 * 60));
