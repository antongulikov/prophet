...
class TestBatcher {
    ...
    std::map<NewCodeMapTy, unsigned long > sch;
...
/**
 * tries to found a fix and store it to res
 * @param candidate_limit
 * @param time_limit
 * @param res
 * @param full_synthesis
 * @param quit_with_any
 * @return
 */
bool ExprSynthesizer::workUntil(size_t candidate_limit, size_t time_limit,
    ...
    while (q.size() > 0 && ((tested_cnt < candidate_limit) || (candidate_limit == 0))) {
        ...
        // this is block for doing old tasks in batcher and renew score
        ...
        if (TB.hasResult()) {
            ...
            if (tmp.size() > 0)
                // if we found score that more than found_score
        ...
        // check if we can pass candidate to one of the testers
        bool found = false;
        ...
        // inc rank
        auto atoms = candidate.atoms;
        for (auto it = atoms.begin(); it != atoms.end(); it++) {
            ranks.push_back(std::make_pair(cnt, candidate.scoreMap[*it]));
        }

        // if we failed passing candidate, we print it
        if (!found) {
    ...
    if (TB.hasResult()) {
        ...
        // TODO fix, too hacky
        for (size_t i = 0; i < succs.size(); i++)
            for (size_t j = i; j < succs.size(); j++)
                if (succs[i].score < succs[j].score)
                    std::swap(succs[i], succs[j]);

        for (size_t i = 0; i < succs.size(); i++) {
            cache->markSucc(succs[i].toString(M));
            unsigned long schema_id = candidate_to_id[succs[i].toString(M)];
            schs.push_back(schema_id);
            outlog_printf(1, "Generate a candidate with schema id: %lu\n", schema_id);
            if (generate_min_id > schema_id)
                generate_min_id = schema_id;
        }
