...
struct RepairCandidate {
   ...
   /* size_t score;
    ...
    std::set<clang::Expr*> getCandidateAtomsWithSCM(SourceContextManager &M) {
            std::set<clang::Expr*> ret;
            ret.clear();
            ret.insert(NULL);
            clang::ASTContext *ctxt = M.getSourceContext(actions[0].loc.filename);
            ...
            for (size_t i = 0; i < actions.size(); i++)
                if (actions[i].kind == RepairAction::ExprMutationKind) {
                    for (size_t j = 0; j < actions[i].candidate_atoms.size(); j++) {
                        // to prevent different atoms to appear
                        std::string str = stmtToString(*ctxt, actions[i].candidate_atoms[j]);
                        if (strs.count(str) == 0) {
                            strs.insert(str);
                            ret.insert(actions[i].candidate_atoms[j]);
                        }
